...
- **每助手配置来自 Aegra assistant 的 `config` 字段** — 创建 assistant 时把设置写进
  `config.configurable`，Aegra 在 run 时并入，工厂据此按助手装配（无磁盘、无需 assistant_id）：
  - `OPENAI_BASE_URL` / `OPENAI_API_KEY` / `OPENAI_MODEL` → 该助手模型（缺项回退全局 env）
  - `prompt` → 该助手系统提示（缺省回退内置 `SYSTEM_PROMPT`）
  - `mcpServers` → MCP 服务器（内联，载入为 tools）
  - `skills` → skills 源目录路径列表（主机路径）

**按 assistant 构图（graph 工厂）**：aegra.json 指向异步工厂
[`make_graph(config)`](agentos/graph.py)，Aegra 每次请求用该 assistant 的 `config` 调用它。
backend 用 deepagents 的 `CompositeBackend`：`default` = 每线程沙箱（含 `execute`，按
`thread_id` 运行时隔离）；每个 `skills` 源目录挂到 `/skills/<name>/` 供 agent 读取与
SkillsMiddleware 渐进式加载（[agentos/builder.py](agentos/builder.py)）。

> Aegra 不向工厂注入 `assistant_id`（只有 `thread_id`/`run_id` + 合并后的 `assistant.config`），
> 因此按助手的隔离一律走 `config` 字段而非按 id 的磁盘目录；密钥只存在 assistant `config` 里，
> 不进 agent 文件系统。
...
│   ├── mcp.py              # 从 mcpServers 载入 MCP 工具（parse / tools）
│   ├── tools.py            # internet_search（Tavily）+ export_artifact 工具
│   ├── sandbox.py          # 每线程沙箱池 + reaper（SessionSandbox / SandboxManager）
│   ├── builder.py          # 组装：backend 组合 + 子代理 + create_deep_agent
│   ├── graph.py            # 异步工厂 make_graph(config) 按 assistant 构图  ← 入口
│   ├── artifacts.py        # 会话产物落盘（thread 作用域，供下载路由）
│   └── routes.py           # Aegra 自定义 HTTP：/files 产物下载（aegra.json http.app）
└── scripts/
...
// POST /assistants 的 config 字段
{
  "configurable": {
    ...
    "OPENAI_API_KEY": "sk-...",
    "OPENAI_MODEL": "gpt-4o",
    "prompt": "You are ...",                        // 该助手系统提示
    "mcpServers": {                                  // 见「接入 MCP 工具」
      "mspbots": { "transport": "streamable_http", "url": "https://host/mcp" }
    },
    "skills": ["/srv/skills/finance"]               // skills 源目录（见「加 skill」）
  }
}
```

缺省项回退全局 env / 内置默认。密钥只存在 assistant `config`，不落磁盘、不进 agent 文件系统。

### 环境变量（`.env`，全局兜底）

per-assistant 的 model/prompt/MCP/skills 在 assistant `config`（见上）；env 仅作全局默认。

| 变量 | 用途 |
| ------------------- | --------------------------------------------------- |
| `OPENAI_BASE_URL`   | 全局网关 base URL（每助手可在 assistant config 覆盖；作兜底） |
| `OPENAI_API_KEY`    | 全局网关 key（无鉴权本地网关可填任意值） |
| `AGENTOS_MODEL`     | 全局默认模型名（默认 `gpt-4o`；每助手可在 assistant config 覆盖） |
...
**加子代理** — 往 `agentos/builder.py` 的 `_subagents(llm)` 追加一个 dict
（`llm` 为工厂构造的该助手模型）：
...
**加 skill**（Agent Skills 规范）— 在该助手 `config.skills` 列出的某个源目录下建
`<source>/<name>/SKILL.md`（`<name>` 须与目录名一致；工厂把每个源目录挂到 `/skills/<源目录名>/`）：
...
**接入 MCP 工具**（QuickBooks / ClickUp / 自建）— 在该助手 `config.configurable.mcpServers`
写 server，graph 工厂自动载入为 tools。每个值遵循 langchain-mcp-adapters 的 Connection
schema（`transport` + 对应字段）：
