...
- **质量 score**:rubric 自审裁决(每 run)与 `/feedback` 用户反馈经 Langfuse SDK 上报为 score,复用同一套 `LANGFUSE_*` 凭据、按 OTEL trace_id 关联到对应 trace(缺凭据则静默禁用,不影响 run)。score 是 Langfuse 一等对象,无法走 OTEL span attribute,故用 SDK 直连(v4 起须开 `tracing_enabled=True` 才会上报,配一个隔离的 TracerProvider 承接其 span processor——既不劫持全局 provider、也不把 aegra 的 span 二次导出)。
